...
pub trait ReadBuffer {
  ...
  fn peek_remaining(&self) -> &[u8] { self.peek_next(self.remaining()) }
...
impl<'a> RefReadBuffer<'a> {
  pub fn new(buff: &[u8]) -> RefReadBuffer { RefReadBuffer { buff, pos: 0 } }
}
...
impl<'a> ReadBuffer for RefReadBuffer<'a> {
  ...
  fn truncate(&mut self, amount: usize) { self.buff = &self.buff[..self.buff.len() - amount]; }
...
impl OwnedReadBuffer {
  pub fn new(buff: Vec<u8>) -> OwnedReadBuffer {
    let len = buff.len();
    OwnedReadBuffer { buff, len, pos: 0 }
  }
...
impl OwnedWriteBuffer {
  pub fn new(buff: Vec<u8>) -> OwnedWriteBuffer {
    let len = buff.len();
    OwnedWriteBuffer { buff, len, pos: 0 }
  }
