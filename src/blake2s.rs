...
impl Blake2s {
  fn set_lastnode(&mut self) { self.f[1] = 0xFFFFFFFF; }
  ...
  pub fn blake2s(out: &mut [u8], input: &[u8], key: &[u8]) {
    let mut hasher: Blake2s = if key.len() > 0 {
...
impl Mac for Blake2s {
   ...
   */
  fn raw_result(&mut self, output: &mut [u8]) { self.finalize(output); }
  ...
  fn test_blake2s_digest() {
    let tests = vec![
      ...
      Test {
        ...
        output: vec![
          0x3f, 0xb7, 0x35, 0x06, 0x1a, 0xbc, 0x51, 0x9d, 0xfe, 0x97, 0x9e, 0x54, 0xc1, 0xee, 0x5b,
          0xfa, 0xd0, 0xa9, 0xd8, 0x58, 0xb3, 0x31, 0x5b, 0xad, 0x34, 0xbd, 0xe9, 0x99, 0xef, 0xd7,
          0x24, 0xdd,
        ],
...
mod mac_tests {
  ...
  fn test_blake2s_mac() {
    ...
    let expected = [
      0x0e, 0x88, 0xf6, 0x8a, 0xaa, 0x5c, 0x4e, 0xd8, 0xf7, 0xed, 0x28, 0xf8, 0x04, 0x45, 0x01,
      0x9c, 0x7e, 0xf9, 0x76, 0x2b, 0x4f, 0xf1, 0xad, 0x7e, 0x05, 0x5b, 0xa8, 0xc8, 0x82, 0x9e,
      0xe2, 0x49,
    ];
